...
class Sales_model extends MY_Model
{
	...
	public $select_column = ['s.id', 's.cust_name', 's.mobile', 's.sell_price', 'i.imei', 's.create_date', 'p.model', 'b.b_name', 'p.sell_status', '(s.sell_price - p.price) profit', 'p.imei_id'];
	// public $order = ['s.id' => 'DESC'];
	public $order = ['s.create_date' => 'ASC'];

	public function make_query()
	{  
		$this->db->select($this->select_column)
            	 ->from($this->table)
				 ->where(['p.is_deleted' => 0, 'p.sell_status' => 1])
                 ->join("purchases p", 'p.id = s.id')
                 ->join("imeis i", 'i.id = p.imei_id')
				 ->join("brands b", 'b.id = p.brand');
		
		if ($this->input->get('brand_id')) $this->db->where_in('p.brand', explode(',', $this->input->get('brand_id')));
